...
const BlogCard = ({ blogImg, title, author, publishedAt, content, slug }) => {
  ...
  const newTitle = title || "Blog Post";

  return (
    <>
      <PageTitle
        currentPageTitle={newTitle}
        currentPage={`${blogNavigation} / BlogPost`}
        prevPage="Home"
      />
        ...
        <article className="prose lg:prose-xl">
          {blogImg && (
            <Image
              ...
              alt="Blog Post"
              className="w-full rounded-lg shadow-lg duration-300 transition-all hover:scale-[1.011]"
              priority
              placeholder="blur"
          ...
          <div
            className={`text-gray-600 flex items-center flex-col lg:flex-row my-4 space-y-1.5 lg:space-y-0 lg:my-2`}
          >
            <span className="px-1">
              <i className="fa-regular fa-clock clock_gradient cursor-pointer"></i>{" "}
              {formatDate(publishedAt)}{" "}
            </span>{" "}
            by {author?.name || "printpropackaging"}
            <span className="flex">
              <svg
                className="mx-2 h-5 w-5 fill-current opacity-75 gradient"
                viewBox="0 0 576 512"
              >
          ...
          <div className="my-20 leading-[2] headings">
